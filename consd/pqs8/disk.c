...
#define PQS8
#ifdef PQS8
#define BLKSIZE	128		/* PQS8 and similar */
#else
#define BLKSIZE 256		/* OS/8 and similar */
#endif
#define BSCALE (256/BLKSIZE)

#ifdef PQS8
#define CSD_MAX		8	/* maximum number of csd images */
#else
#define CSD_MAX		4	/* maximum number of csd images */
#endif
#define CSD_BLKCNT	4095	/* max image block count */
int csd_dsk[CSD_MAX][CSD_BLKCNT][BLKSIZE]; /* consolidated csd images */
char csd_name[CSD_MAX][256];	/* file name table */
FILE *csd_stream[CSD_MAX];	/* csd streams */

#define TC_BLKCNT	737*BSCALE /* block count of a DECtape */
int tc_dsk[TC_BLKCNT][BLKSIZE];	/* pointer to the DECtape image */
char tc_name[256];		/* file name */
FILE *tc_stream;		/* TC0X stream */

#define RK_BLKCNT	3248	/* block count of an RK05 image */
int rk_dsk[2][RK_BLKCNT][BLKSIZE]; /* RK05 platter as a memory image */
...
int patch_bt;	/* flag to indicate if the boot block needs to be patched with the handler */
  ...
  for( d=0; d<CSD_MAX; d++ ){
    ...
    if( csd_stream[d]==NULL ){
      ...
      for( blk=0; blk<CSD_BLKCNT; blk++ ){	/* read through all the blocks */
        for( i=0; i<BLKSIZE; i++ ){			/* fetch all the words in the block */
  ...
  if( rk_stream==NULL ){
    ...
    for( half=0; half<2; half++){
      for( blk=0; blk<RK_BLKCNT; blk++ ){	/* read through all the blocks */
        for( i=0; i<BLKSIZE; i++ ){		/* fetch all the words in each block */
  ...
  case 231229:	/* PQS8 DECtape handler */
    ...
    break;
  case 223621:	/* PQS8 RX01 handler */
    fprintf( stderr, "SERVER: %s appears to have the PQS8 RXA handler cksum=%d.\r\n", csd_name[0], k );
    patch_bt=1;	/* Need to patch the boot block */
    break;
  default:
    fprintf( stderr, "SERVER: Unknown Image type for file %s cksum=%d.\r\n", csd_name[0], k );
    patch_bt=1;	/* Need to patch the boot block */
    break;
#else
#ifdef OS8
...
int i,j,k;	/* indexes and temps */
  ...
  if( rk_stream != NULL ){
    ...
    for( half=0; half<2; half++){
      for( blk=0; blk<RK_BLKCNT; blk++ ){	/* all the blocks */
        for( i=0; i<BLKSIZE; i++ ){		/* all the words in each block */
  ...
  if( tc_stream != NULL ){
    ...
    for( blk=0; blk<TC_BLKCNT; blk++ ){	/* all the blocks */
      ...
      fputc( 0, tc_stream );
      for( i=128; i<BLKSIZE; i++ ){		/* all the words in each block */
        j=tc_dsk[blk][i];		/* get the word */
        fputc( j&0377, tc_stream );
        fputc( (j>>8)&0377, tc_stream );
      }
